...
class LocalGConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # support_reshape = support.reshape(batch_size, num_pt, -1)
        support_loop = torch.matmul(inputs, self.loop_weight)
        support_loop_reshape = support_loop.reshape(batch_size, num_pt, num_hypo, -1)
        # support_loop_reshape = support_loop.reshape(batch_size, num_pt, -1)

        output = self.adj_mat @ support_reshape  # + support_loop_reshape
        output = output + support_loop_reshape
        # output = dot(self.adj_mat, support_reshape, True)
        # output = output + support_loop_reshape
        # output = output.reshape(batch_size, num_pt, num_hypo, -1)
