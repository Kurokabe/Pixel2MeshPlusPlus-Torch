...
class Ellipsoid(object):
    def __init__(self, mesh_pos: List[float], file=config.ELLIPSOID_PATH):
        with open(file, "rb") as fp:
            fp_info = pickle.load(fp, encoding="latin1")

        # shape: n_pts * 3
        self.coord = torch.tensor(fp_info[0]) - torch.tensor(
            mesh_pos, dtype=torch.float
        )
        ...
        self.unpool_idx = [
            torch.tensor(fp_info[4][i], dtype=torch.long) for i in range(2)
        ]
